...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func handleOpenUrl(url: URL){
        ...
        TwitterAPICaller.client?.fetchAccessToken(withPath: "oauth/access_token", method: "POST", requestToken: requestToken, success: { (accessToken: BDBOAuth1Credential!) in
            self.loginSuccess?()
            print(accessToken.userInfo as NSDictionary)
            let user = accessToken.userInfo as NSDictionary
    ...
    func getUserImageDictionary(url: String, parameters: [String:Any], success: @escaping ([String : Any]) -> (), failure: @escaping (Error) -> ()){
           print(parameters)
           TwitterAPICaller.client?.get(url, parameters: parameters, progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
   ...
   // https://api.twitter.com/1.1/followers/ids.json
    ...
    func getFollowers(url: String, parameters: [String:Any], success:@escaping ([String : Any]) -> (), failure: @escaping (Error) -> ()){
        print(parameters)
        TwitterAPICaller.client?.get(url, parameters: parameters, progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success(response as! [String : Any])
        }, failure: { (task: URLSessionDataTask?, error: Error) in
    ...
    func getCurrentUserData(url: String, parameters: [String:Any], success:@escaping ([String: Any]) -> (), failure: @escaping (Error) -> ()){
        print(parameters)
        TwitterAPICaller.client?.get(url, parameters: parameters, progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success(response as! [String: Any])
        }, failure: { (task: URLSessionDataTask?, error: Error) in
